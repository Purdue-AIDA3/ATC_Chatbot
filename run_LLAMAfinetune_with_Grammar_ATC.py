...
def main(test_mode=False):
    ...
    V_ATC_ids = torch.load("V_ATC_ids_llama2.pt")
    ...
    loss_data_path = f"loss-data-llama-grammar-loss{re.sub(r'[ ()]', '', CHOOSE_MODEL) }.json"
    ...
    test_data_path = f"atc-test-data-with-response-Llama-grammar-loss{re.sub(r'[ ()]', '', CHOOSE_MODEL) }.json"
    with open(test_data_path, "w") as file:
        json.dump(test_data, file, indent=4)  # "indent" for pretty-printing
    print(f"Responses saved as {test_data_path}")

    file_name = f"{re.sub(r'[ ()]', '', CHOOSE_MODEL) }-atc-Llama-grammar-loss.pth"
